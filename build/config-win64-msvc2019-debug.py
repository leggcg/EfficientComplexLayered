BUILDDIR        = '#build/debug_msvc2019'
DISTDIR         = '#dist/debug_msvc2019'
CXX             = 'cl'
CC              = 'cl'
CXXFLAGS        = ['/nologo', '/Od', '/Z7', '/fp:fast', '/D', 'WIN32', '/D', 'WIN64', '/W3', '/EHsc', '/GS-', '/GL', '/MD', '/D', 'MTS_DEBUG', '/D', 'SINGLE_PRECISION', '/D', 'SPECTRUM_SAMPLES=3', '/D', 'MTS_SSE', '/D', 'MTS_HAS_COHERENT_RT', '/D', '_CONSOLE', '/D', 'DEBUG', '/D', 'OPENEXR_DLL', '/openmp', '/FC', '/std:c++latest']
SHCXXFLAGS      = CXXFLAGS
TARGET_ARCH     = 'x86_64'
MSVC_VERSION    = '14.2'
LINKFLAGS       = ['/nologo', '/SUBSYSTEM:CONSOLE', '/DEBUG', '/MACHINE:X64', '/FIXED:NO', '/OPT:REF', '/OPT:ICF', '/LTCG', '/NODEFAULTLIB:LIBCMT', '/MANIFEST']
BASEINCLUDE     = ['#include', '#dependencies/include']
BASELIB         = ['msvcrt', 'ws2_32', 'Half', 'zlib']
BASELIBDIR      = ['#dependencies/lib']
OEXRINCLUDE     = ['#dependencies/include/openexr']
OEXRLIB         = ['IlmImf', 'IlmThread', 'Iex', 'zlib']
BOOSTLIB        = ['boost_system-vc141-mt-x64-1_69', 'boost_filesystem-vc141-mt-x64-1_69', 'boost_thread-vc141-mt-x64-1_69']
# COLLADAINCLUDE  = ['#dependencies/include/collada-dom', '#dependencies/include/collada-dom/1.4']
# COLLADALIB      = ['libcollada14dom24']
XERCESLIB       = ['xerces-c_3']
PNGLIB          = ['libpng16']
JPEGLIB         = ['jpeg']
GLLIB           = ['opengl32', 'glu32', 'glew32mx', 'gdi32', 'user32']
GLFLAGS         = ['/D', 'GLEW_MX']
SHLIBPREFIX     = 'lib'
SHLIBSUFFIX     = '.dll'
LIBSUFFIX       = '.lib'
PROGSUFFIX      = '.exe'
PYTHON27LIB     = ['boost_python27-vc141-mt-x64-1_69', 'python27']
PYTHON27INCLUDE = ['#dependencies/include/python27']
PYTHON36LIB     = ['boost_python36-vc141-mt-x64-1_69', 'python36']
PYTHON36INCLUDE = ['#dependencies/include/python36']
PYTHON37LIB     = ['boost_python37-vc141-mt-x64-1_69', 'python37']
PYTHON37INCLUDE = ['#dependencies/include/python37']
QTINCLUDE       = ['#dependencies/include']
QTDIR           = '#dependencies'
FFTWLIB         = ['libfftw3-3']
