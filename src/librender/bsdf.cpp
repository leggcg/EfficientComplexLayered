...
/// For Guo2018
Spectrum BSDF::evalguo2018(const BSDFSamplingRecord &bRec, Float &_pdf, EMeasure measure) const {
	_pdf = pdf(bRec, measure);
	return eval(bRec, measure);
}
void BSDF::evalAndSample(BSDFSamplingRecord &bRec, Spectrum &evalVal, Float &evalPdf, Spectrum &sampleVal, Float &samplePdf, const Point2 &nextSample, EMeasure measure) const {
	const BSDFSamplingRecord _bRec(bRec);
	evalVal = evalguo2018(_bRec, evalPdf, measure);
