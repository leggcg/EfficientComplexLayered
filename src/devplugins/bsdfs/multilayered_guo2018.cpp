...
public:
	...
	void configure() {
		...
		if (m_bidir) {
			...
			cout << "[GY]: Using unidir Eval ";
			if (m_MISenable) {
				cout << "(MIS)" << endl;
			}
			else {
				cout << "(not using MIS)" << endl;
			}
		}
		...
		Properties interiorAniHomoProps("homogeneous_aniso_guo2018");
		...
		// this creates a whole set of duplicates of the same layered configuration
	...
	// LEGG bug fix 
	Normal getNormalFromTexture(const ref<Texture2D> normal_texture, const Intersection &its) const {
		Normal normal;
		//normal_texture->eval(uv).toLinearRGB(normal.x, normal.y, normal.z);
		normal_texture->eval(its,false).toLinearRGB(normal.x, normal.y, normal.z);
		for (int i = 0; i < 3; ++i) {
	...
	// query the different textures in the layers
	// THIS ASSUMES SAME UV POSITION as the intersection point FOR ALL LAYERS
	void setParameters(const BSDFSamplingRecord &_bRec, std::vector<Frame> &frames,
		...
		for (int l = 0; l < m_nbLayers-1; ++l) {
			if (m_flag_aniso[l]) {
				...
				if (m_flag_sigmaTs[l]) sigmaT *= m_texture_sigmaTs[l]->eval(_bRec.its,false); // LEGG bugfix
				Spectrum albedo = m_spectrum_albedos[l];
				//if (m_flag_albedos[l]) albedo *= m_texture_albedos[l]->eval(uv);
				if (m_flag_albedos[l]) albedo *= m_texture_albedos[l]->eval(_bRec.its,false); // LEGG bugfix
		...
		for (int l = 0; l < m_nbLayers; ++l) {
			...
			const Normal normal = m_flag_normals[l] ? getNormalFromTexture(m_texture_normals[l], _bRec.its) : m_vector_normals[l];
	...
	// Returns true if ray intersects any of the layers
	bool rayIntersect(const Ray &ray, Intersection &its) const {
		...
		// ray direction is too "horizontal"?
		if (std::abs(ray.d.z) < Epsilon)
			return false;

		// is the ray pointing up? (leaving the surface)
		bool isUp = ray.d.z > 0 ? true : false;
		
		Float z = ray(Epsilon).z;

		// find the two nearest integer numbers that enclose z
		Float z_bot = std::floor(z);
		Float z_top = z_bot + 1;

		// is the ray outside the surface and escaping?
		if ((z_top > 0 && isUp) || (z_bot < -(m_nbLayers - 1) && !isUp)) {
			return false;
		}

		// layers are numbered 0,-1,-2,...,-(m_nbLayers-1)
		// is the ray positioned outside the [-(n_nbLayers-1),0] interval?
		...
		// x and y at the next interface
		// Equation 4 (zp-z)/cosw = hypotenuse (aka distance travelled)
	...
	// same as rayIntersect, but sets isEmitter to true if the intersection is at:
	//	the top layer (flag_type true)
	//	the bottom layer (flag_type false)
	...
	// pretty much a standard refraction sample, except for the Jacobian
	...
	// standard eval Refraction, except for the Jacobian
	...
	struct PathInfo {
		Point p;
		Vector wi, wo;
		int topCounter, bottomCounter; // times the path reached top and bottom layers so far
		bool surf; // true=surface, false=medium
		int layerID; // layer number
		Spectrum thru0, thru1; // throughput before and after sampling
		Vector2 vpdf, epdf; // 0 is fwd, 1 is reverse, vpdf direction pdf, epdf distance pdf
		Float pSurvival; // RR survival
	};
	...
	// Returns the throughput of the path starting at _bRec.wi 
	// - stores outgoing direction in _bRec.wo
	// - the path is saved in path
	Spectrum generatePath(BSDFSamplingRecord &_bRec,
		...
		// ray originates 1 unit away from originPoint (wi is normalized) in the inverse direction
		...
		while (depth < maxDepth || maxDepth < 0) {
			...
			if (flag_medium && mediums[curLayer]->sampleDistance(Ray(ray, 0, its.t), mRec, sampler)) {
				...
				// its.p.z has the intersected layer but in [-n+1,0], change it to [0,n-1]
	...
	void unidirEvaluation(const BSDFSamplingRecord &_bRec,
		...
		// isn't it easier to do flag_neeDir = _bRec.wo.z > 0 ???
		...
		for (size_t i = 0; i < depth; ++i) {
			...
			if (!path[i].surf) {// medium
				...
				if ((flag_neeDir && path[i].layerID == 0) || (!flag_neeDir && path[i].layerID == (m_nbLayers-2))) {
					...
					// get top or bottom layers accordingly
				...
				// Direct
				// wi.z>0 && reflection && toplayer && 1st time interaction with top layer ||
				// wi.z<0 && reflection && bottomlayer && 1st interaction with bottom layer
				if ((flag_incidentDir && flag_type && curLayer == 0 && path[i].topCounter == 1) ||
					...
					// explicitly connect path up to this point (thru0) in wo direction
					if (mode == 1) Li += path[i].thru0 * bsdf->eval(bRec);
					if (mode == 2) pdf += bsdf->pdf(bRec);
				}

				// wiz>0 && reflection && 2nd top layer ||
				// wiz>0 && transmission && 2nd bottom layer ||
				// wiz<0 && reflection && 2nd bottom layer ||
				// wiz<0 && transmission && 2nd top layer
				// in concrete: we are one layer away from target layer at wo
				if ((flag_incidentDir && flag_type && curLayer == 1) ||
					...
					// flag_neeDir wo on top (1), wo on bottom (0)
					// get appropiate top or bottom layer info
					const BSDF *bsdf_nee = flag_neeDir ? m_bsdfs[0].get() : m_bsdfs[m_nbLayers - 1].get();
					const Frame frame_nee = flag_neeDir ? frames[0] : frames[m_nbLayers - 1];
					const Medium *medium = flag_neeDir ? mediums[0].get() : mediums[m_nbLayers - 2].get();
					// use wo as input direction to sample a refracted direction into the material
					Spectrum throughput_refract = sampleRefraction(_bRec, bsdf_nee, frame_nee,
						_bRec.wo, wo_refract, refractPdf, refractPdf_re);

					if (!throughput_refract.isZero()) { // fig 7a
						Intersection its_nee;
						Ray ray_nee = Ray(path[i].p, -wo_refract, 0.0);
						if (!rayIntersect(ray_nee, its_nee)) {
						...
						Spectrum value(0.0);
						// evaluate transmittance until the next layer
						if (mode == 1) value = medium->evalTransmittance(Ray(ray_nee, 0.0, its_nee.t), sampler);
						if (mode == 2) 
						{ 
							medium->eval(Ray(its_nee.p, wo_refract, 0.0, its_nee.t, 0.0), mRec); 
							// LEGG: fixes bug for computing pdf (mode 2)
							value=Spectrum(1.0); 
						}
					...
					if (m_MISenable) { // fig7b
						...
						if (isEmitter) { // not an actual emitter more like "boundary layer"
							const BSDF *bsdf = flag_neeDir ? m_bsdfs[0].get() : m_bsdfs[m_nbLayers - 1].get();
							const Frame frame = flag_neeDir ? frames[0] : frames[m_nbLayers - 1];
	...
	// path_L is forward (from camera)
	// path_R is backwards (from light)
	void bidirEvaluation(const BSDFSamplingRecord &_bRec,
		...
		// get the frame at the top or bottom layer (where wo exits)
		...
		for (size_t i = 0; i < (mode == 1 || m_stochPdfDepth < 0 ? path_L.size() : std::min(path_L.size(), size_t(m_stochPdfDepth))); ++i) {
			for (size_t j = 0; j < (mode == 1 || m_stochPdfDepth < 0 ? path_R.size() : std::min(size_t(m_stochPdfDepth) - i, path_R.size())); ++j) {
				...
				if (path_L[i].surf) { // surface
					// z2 in (z1-1,z1+1) and z1!=z2
					...
					// z2 in (floor(z1),ceil(z1)), i.e. in the same medium layer
					...
					if (!path_R[j].surf || (path_R[j].wi.z*frame.toLocal(path_R[j].wi).z > 0 && -path_L[i].wo.z*frame.toLocal(-path_L[i].wo).z > 0)) {
						...
						if (t > Epsilon) {
							...
							if (mode == 2 || !funcVal.isZero()) {
								if (mode == 1) f = path_L[i].thru1 * path_R[j].thru0 * funcVal * mRec.transmittance / std::abs(path_L[i].wo.z); // denominator is Jacobian of Eq.9
	...
	// pdf() method invokes this function with mode=1, actual result in samplePdf
	// _bRec is for sample(), bRec is for eval()
	void pdfEvaluation(const BSDFSamplingRecord &_bRec, const BSDFSamplingRecord &bRec,
		...
		if (m_pdfMode == "const") {
		...
		else if (m_pdfMode == "bidirStochTRT") { // approximate PDF method in section 5.3.2
			...
			for (int i = 0; i < m_pdfRepetitive; ++i) {
				...
				// backwards=false, bidir=true
				// sample a path from _bRec.wi and outgoing direction stored in bRec_tmp.wo
				// 	(_bRec.wi = bRec_tmp.wi)
				generatePath(bRec_tmp, frames, mediumsForPdf, m_stochPdfDepth, path, ratio, ratioPdf, false, true);

				Float _samplePdf = 0.0;
				Float _evalPdf = 0.0;
				if (mode == 1 || mode == 3) {
					// sample a path from _bRec.wo and outgoing direction bRec_tmp.wo
					bRec_tmp.wi = _bRec.wo;
					// backwards=true, bidir=true
					...
					// join "path" (starting at _bRec.wi) with "path_R_sample" (start _bRec.wo)
	...
	void evalAndSample(BSDFSamplingRecord &_bRec, Spectrum &evalVal, Float &evalPdf, 
				Spectrum &sampleVal, Float &samplePdf,
		...
		// get shading normal (store in frames)
		// get medium properties from texture (store in mediums): 
		//	- density, albedo, orientation for anisotropic
		//	- sigmaT,albedo otherwise
		setParameters(bRec, frames, mediums);

		Float evalPdf_tmp = 0.0; 
	
		// no transmission && (wi or wo are in the lower hemisphere)	
		if (!(BSDF::getType() & BSDF::ETransmission) && (Frame::cosTheta(bRec.wi) <= 0 || Frame::cosTheta(bRec.wo) <= 0)) {
			...
			else { // wi is in the upper hemisphere
				// sample 
				{
					std::vector<PathInfo> path;
					std::vector<Float> ratio, ratioPdf;
					// get path throughput, flagbackward=false, flagbidir=false
		...
		else { // BSDF supports transmission
			{
				// eval, sample, wo, (eval pdf)
				if (m_bidir) {
					std::vector<PathInfo> path;
					std::vector<Float> ratio, ratioPdf;
					// path throughput, flagbackward=false, flagbidir=true
					...
					bRec_tmp.wi = bRec.wo;
					// ignore path throughput, flagbackward=true, flagbidir=true
